...
def checkDay(givenDate):
    ...
    specialDaysNumbers = [905, 926, 1005, 1021, 1024, 1100, 1104, 1108, 1123, 1124, 1125, 1222, 1223, 1224, 1225, 1226, 1227, 1228, 1229, 1230, 1231, 1100, 1200, 10300, 11600, 11700, 12300, 22000, 31700, 32400, 4100, 4200, 4300, 4400, 4500, 4600, 4700, 4800, 4900, 41000, 42100, 52900, 61900, 62000, 62100]
    ...
    startDayFull = ['9', '2', '2022'] # Testing date, is an A day

    startMonth = startDayFull[0]
    startDay = startDayFull[1]
    startYear = startDayFull[2]

    #if "/" in givenDate == False:
        #givenDate = givenDate.strftime("%m/%d/%Y")
    ...
    month = parsedToday[0]
    day = parsedToday[1]
    # Change date so that it's always two digits
    if len(day) == 1:
        day = "0" + day
    ...
    for index, key in enumerate(specialDays):
        ...
        #print("key: " + str(index))
        #print("currentDayCheckNumber: " + str(currentDayCheckNumber))
        #print("specialDaysNumbers: " + str(specialDaysNumbers[index]))
        #print(specialDays[key][1])

        if currentDayCheckNumber >= specialDaysNumbers[index]:
            
            print("Value of dayAdjust before subtracting: " + str(dayAdjust))
...
@plugin.command
@lightbulb.command('tdy', "Tells you if today is an A day or B day.")
...
async def user_data(ctx):
    ...
    print(f"User has requested: {currentDate}")

    date = checkDay(str(currentDate))
